...
if("RMark"%in%row.names(installed.packages())){
    ...
    mle.fake <- mark(data=fake.processed,ddl=fake.ddl,model="CJS",model.parameters=list(Phi=list(formula=~Flood*sex+fake1+fake2+fake3+fake4+fake5),p=list(formula=~Flood*sex+fake1+fake2+fake3+fake4+fake5)),delete=TRUE,retry=30,output=FALSE,invisible=TRUE,silent=TRUE)
    # poor convergence
